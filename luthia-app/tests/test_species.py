...
Exercises /api/species (list + search + filters) and /api/species/<id> (detail)
...
def test_api_species_list_returns_200_with_required_keys(client, seed_db):
    """GET /api/species must return HTTP 200 with at least 'rows' and 'total' keys.
...
def test_api_species_list_includes_seeded_species(client, seed_db):
    """Unfiltered /api/species must include the seeded European Ash species."""
    data = client.get('/api/species').get_json()
    assert data['total'] >= 1
...
def test_api_species_search_no_match_returns_empty(client, seed_db):
    ...
    response = client.get('/api/species?q=zzznomatch')
    assert response.status_code == 200
    data = response.get_json()
    assert data['total'] == 0
    assert data['rows'] == []
...
def test_api_species_cites_filter_returns_zero_for_seed_data(client, seed_db):
    """GET /api/species?cites=1 must return total=0 because the seed species is
    ...
    ignored.
    """
    response = client.get('/api/species?cites=1')
    assert response.status_code == 200
    data = response.get_json()
    assert data['total'] == 0
...
def test_api_species_detail_returns_200_with_scientific_name(client, seed_db):
    """GET /api/species/<valid_id> must return 200 and a payload containing
...
def test_api_species_detail_404_for_unknown_id(client, seed_db):
    """GET /api/species/<unknown_id> must return 404, not a 500 server error."""
