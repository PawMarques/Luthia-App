...
Exercises GET / POST / PATCH / DELETE on /api/vendors using the Flask test
...
def test_api_vendors_returns_200_with_nordic_woods(client, seed_db):
    """GET /api/vendors must return HTTP 200 and include 'Nordic Woods' from seed data.
...
def test_api_vendor_create_valid_returns_201(client, seed_db):
    """POST /api/vendors with a valid payload must create the record and return 201.
...
def test_api_vendor_create_duplicate_name_returns_400(client, seed_db):
    """POST /api/vendors with the existing name 'Nordic Woods' must return 400.
...
def test_api_vendor_create_empty_name_returns_400(client, seed_db):
    """POST /api/vendors with an empty name must return 400 with a validation error.
...
def test_api_vendor_update_country(client, seed_db):
    """PATCH /api/vendors/<id> with a new country must update the field and return ok=True.
...
# ---------------------------------------------------------------------------
# DELETE /api/vendors/<id>  (soft-delete / toggle)
# ---------------------------------------------------------------------------

def test_api_vendor_toggle_active_twice(client, seed_db):
    """DELETE /api/vendors/<id> must toggle the active flag on each call.
    ...
    r1 = client.delete(f'/api/vendors/{vendor_id}')
    ...
    r2 = client.delete(f'/api/vendors/{vendor_id}')
